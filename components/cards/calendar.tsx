...
const Calendar = () => {
  ...
  const generateCalendarDays = () => {
    const daysArray: Array<{day: number, currentMonth: boolean}> = [];

    // Add days from the previous month if the month doesn't start on Sunday
    if (startDayOfMonth > 0) {
      for (let i: number = startDayOfMonth - 1; i >= 0; i--) {
        daysArray.unshift({
    ...
    for (let day: number = 1; day <= daysInCurrentMonth; day++) {
      daysArray.push({
        day,
        currentMonth: true,
      });
    ...
    if (endDayOfMonth < 6) {
      for (let i: number = 1; i <= 6 - endDayOfMonth; i++) {
        daysArray.push({
          day: i,
        ...
        <div className="flex items-center gap-10 mb-10">
          <div className="flex items-center gap-2">
            <motion.button
              ...
              onClick={prevMonth}
              className="size-8 rounded-full flex items-center justify-center bg-muted"
            >
        ...
        <div className="grid grid-cols-7 gap-2 mb-3 text-center font-bold ">
          {daysOfWeek.map((day) => (
            <motion.div
              layout
              key={day}
              className="bg-muted text-muted-foreground text-sm py-1 rounded-full uppercase"
            >
              {day}
            </motion.div>
          ))}
